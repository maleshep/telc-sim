...
function getGrade(points: number): { grade: string; label: string; emoji: string } {
  if (points >= 54) return { grade: 'sehr gut', label: '1 — sehr gut', emoji: '' };
  if (points >= 48) return { grade: 'gut', label: '2 — gut', emoji: '' };
  if (points >= 42) return { grade: 'befriedigend', label: '3 — befriedigend', emoji: '' };
  if (points >= 36) return { grade: 'ausreichend', label: '4 — ausreichend', emoji: '' };
  return { grade: 'nicht bestanden', label: '5 — nicht bestanden', emoji: '' };
}

export function Results({ result, onRestart, onHome }: ResultsProps) {
